...
const vec4 light_color = vec4(.88, .73, 1.0, .5); //r, g, b, a. the gradient light that is above (and on top) of the water
const vec4 glow_color = vec4(.88, .73, 1.0, .5); //alpha is unused here. the color that white parts glow when near water
const float glow_strength = 9;                 //how strong the glow effect is
...
const float playerglow_strength = 0.6;
...
void vert(){
    ...
    gl_FrontColor = vec4(texture2D(palettetex, vec2((outcolor.r*15.0 + .5) / 16.0, (palette + .5) / 64.0)).rgb, outcolor.a);
...
void frag(){
    ...
    vec4 basecolor = texture2D(framebuf, screencoords.xy);

    //reflections
    float waterheight = screensize.y - 200; // 200 pixels of white at screen bottom
    ...
    float angle = dot(vec2(0.0, -1.0), playervec) / (dp) * 100.0;
    float angle2 = dp;

    dp += noise(dp / 32.0 + timer * 2.0) * 6.0;
    dp += noise(dp / 16.0 + timer  * -4.35) * 3.0;
    dp += noise(dp / 8.0 + timer * 1.0)*1.5;

    dp += noise(angle / 32.0 + timer * 2.0) * 6.0;
    dp += noise(angle / 16.0 + timer  * -4.35) * 3.0;
    dp += noise(angle / 8.0 + timer * 1.0)*1.5;
    ...
    dx += noise(worldPos.y / 32.0 + timer * 2.0) * 6.0;
    dx += noise(worldPos.y / 16.0 + timer  * -4.35) * 3.0;
    dx += noise(worldPos.y / 8.0 + timer * 1.0)*1.5;

    float glowval3 = -(dx / 360.0/* * ratio.y*/);
    glowval3 = glowval3*light_distance / (playerglow_strength * 4.0);
    glowval3 += 1.0;
